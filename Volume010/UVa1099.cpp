...
bool can_partition(int x, int s) {
    ...
    for (int S0 = (s-1) & s; S0; S0 = (S0-1) & s) {
        ...
        if (subsum[S0] % x == 0 && subsum[S1] % x == 0) {
            if (can_partition(min(x, subsum[S0]/x), S0) && 
                can_partition(min(x, subsum[S1]/x), S1)) {
                return DP[x][s] = 1;
            }
        }

        if (subsum[S0] % y == 0 && subsum[S1] % y == 0) {
            if (can_partition(min(y, subsum[S0]/y), S0) && 
                can_partition(min(y, subsum[S1]/y), S1)) {
                return DP[x][s] = 1;
            }
        }
    }
...
int main () {
    ...
    while (cin >> n && n) {
        ...
        printf("Case %d: ", Case++);
        if (subsum[full] == x*y && can_partition(x, full)) printf("Yes\n");
        else printf("No\n");
    }
    return 0;
}